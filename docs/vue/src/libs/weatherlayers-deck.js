...
import {
  distance as distance$1,
  destinationPoint as destinationPoint$1,
  initialBearing,
} from "geodesy-fn";
...
import {
  transform,
  factory,
} from "rollup-plugin-worker-factory/src/browser.js";
...
const DATETIME = "2025-09-15T08:34:52.654Z";
...
function formatValue(value, unitFormat) {
  ...
  const roundedFormattedValue = decimals
    ? Math.round(formattedValue * 10 ** decimals) / 10 ** decimals
...
function formatDirection(direction, directionType, directionFormat) {
  ...
  if (directionFormat === DirectionFormat.VALUE) {
    ...
    directionFormat === DirectionFormat.CARDINAL ||
    directionFormat === DirectionFormat.CARDINAL2 ||
    ...
    const index =
      Math.floor(((direction % 360) + cardinalDelta / 2) / cardinalDelta) %
      cardinals.length;
...
async function loadGeotiff(url, options) {
  ...
  const sourceData = await geotiffImage.readRasters({
    interleave: true,
    signal: options?.signal,
  });
  if (
    !(
      ...
      sourceData instanceof Uint8ClampedArray ||
      sourceData instanceof Float32Array
    )
  ) {
...
function loadCached(loadFunction) {
  return async (url, options) => {
    ...
    const cacheKey =
      url + (options?.headers ? ":" + JSON.stringify(options?.headers) : "");
...
function interpolateDatetime(start, end, weight) {
  ...
  if (weight <= 0) {
    ...
    const date = new Date(
      startDate.getTime() + (endDate.getTime() - startDate.getTime()) * weight
    );
...
function offsetDatetime(datetime, hour) {
  ...
  const updatedDatetimeDate = new Date(
    datetimeDate.getTime() + hour * 1000 * 60 * 60
  );
  return updatedDatetimeDate.toISOString();
}
function offsetDatetimeRange(datetime, startHour, endHour) {
  return [
    offsetDatetime(datetime, startHour),
    offsetDatetime(datetime, endHour),
  ];
}
...
function getPixelDirectionValue(pixel, imageType, imageUnscale) {
  if (imageType === ImageType.VECTOR) {
    const value = getPixelVectorValue(pixel, imageType, imageUnscale);
    return (
      (360 - ((Math.atan2(value[1], value[0]) / Math.PI) * 180 + 180) - 270) %
      360
    );
  } else {
    return NaN;
...
function spline(c0, c1, c2, c3, a) {
  const color = add(
    add(
      add(mul(c0, dot(BS_B, powers(a + 1))), mul(c1, dot(BS_A, powers(a)))),
  ...
  color[3] =
    c0[3] > 0 && c1[3] > 0 && c2[3] > 0 && c3[3] > 0
      ? Math.max(Math.max(Math.max(c0[3], c1[3]), c2[3]), c3[3])
      : 0;
  return color;
}
/**
...
function getPixelCubic(image, imageDownscaleResolution, uvX, uvY) {
  ...
  return spline(
    spline(
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, -1, -1),
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, 0, -1),
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, 1, -1),
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, 2, -1),
      fuvX
    ),
    spline(
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, -1, 0),
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, 0, 0),
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, 1, 0),
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, 2, 0),
      fuvX
    ),
    spline(
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, -1, 1),
    ...
    spline(
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, -1, 2),
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, 0, 2),
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, 1, 2),
      getPixel(image, imageDownscaleResolution, iuvX, iuvY, 2, 2),
      fuvX
    ),
    fuvY
  );
...
function getPixelInterpolate(
  ...
  const uvWithOffsetX = isRepeatBounds
    ...
    : mixOne(
        0 + 0.5 / imageDownscaleResolution[0],
        1 - 0.5 / imageDownscaleResolution[0],
        uvX
      );
  const uvWithOffsetY = mixOne(
    0 + 0.5 / imageDownscaleResolution[1],
    1 - 0.5 / imageDownscaleResolution[1],
    uvY
  );
  if (image2 && imageWeight > 0) {
    const pixel = getPixelFilter(
      image,
      imageDownscaleResolution,
      imageInterpolation,
      uvWithOffsetX,
...
function getImageDownscaleResolution(width, height, imageSmoothing) {
  ...
  return [
    width / imageDownscaleResolutionFactor,
    height / imageDownscaleResolutionFactor,
  ];
}
...
function wrapBounds(bounds) {
  // wrap longitude
  const minLng =
    bounds[2] - bounds[0] < 360 ? wrapLongitude(bounds[0]) : MIN_LNG;
  const maxLng =
    bounds[2] - bounds[0] < 360 ? wrapLongitude(bounds[2], minLng) : MAX_LNG;
  // clip latitude
...
function getRasterPoints(imageProperties, bounds, positions) {
  ...
  const imageDownscaleResolution = getImageDownscaleResolution(
    width,
    height,
    imageSmoothing
  );
  ...
  const rasterPoints = positions.map((position) => {
    ...
    const pixel = getPixelInterpolate(
      image,
      image2,
      imageDownscaleResolution,
      imageInterpolation,
    ...
    if (
      (typeof imageMinValue === "number" &&
        !isNaN(imageMinValue) &&
        value < imageMinValue) ||
      (typeof imageMaxValue === "number" &&
        !isNaN(imageMaxValue) &&
        value > imageMaxValue)
    ) {
...
function getRasterMagnitudeData(imageProperties, bounds) {
  ...
  const effectiveImageInterpolation =
...
function createPaletteTexture(device, paletteScale) {
  ...
  const paletteBounds = [
    paletteDomain[0],
    paletteDomain[paletteDomain.length - 1],
  ];
...
function _getCoordinateUniforms(props = {}) {
  ...
  if (imageCoordinateSystem !== DEFAULT) {
    ...
    imageCoordinateSystem =
      imageCoordinateSystem === LNGLAT ? LNGLAT : CARTESIAN;
    if (
      imageCoordinateSystem === LNGLAT &&
      defaultImageCoordinateSystem === CARTESIAN
    ) {
      // LNGLAT in Mercator, e.g. display LNGLAT-encoded image in WebMercator projection
      return { coordinateConversion: -1, bounds };
    }
    if (
      imageCoordinateSystem === CARTESIAN &&
      defaultImageCoordinateSystem === LNGLAT
    ) {
...
function getUniforms$4(props = {}) {
  ...
  return {
    ...
    [tokens$4.transparentColor]: props.transparentColor
      ? deckColorToGl(props.transparentColor)
      : [0, 0, 0, 0],
  };
}
const bitmapModule = {
...
function getUniforms$3(props = {}) {
  return {
    ...
    [tokens$3.imageResolution]: props.imageTexture
      ? [props.imageTexture.width, props.imageTexture.height]
      : [0, 0],
    [tokens$3.imageSmoothing]: props.imageSmoothing ?? 0,
    [tokens$3.imageInterpolation]: Object.values(ImageInterpolation).indexOf(
      props.imageInterpolation ?? ImageInterpolation.NEAREST
    ),
    [tokens$3.imageWeight]:
      props.imageTexture2 !== props.imageTexture && props.imageWeight
        ? props.imageWeight
        : 0,
    [tokens$3.imageType]: Object.values(ImageType).indexOf(
      props.imageType ?? ImageType.SCALAR
    ),
    ...
    [tokens$3.borderColor]: props.borderColor
      ? deckColorToGl(props.borderColor)
      : [0, 0, 0, 0],
    [tokens$3.gridEnabled]: props.gridEnabled ? 1 : 0,
    [tokens$3.gridSize]: props.gridSize ?? 0,
    [tokens$3.gridColor]: props.gridColor
      ? deckColorToGl(props.gridColor)
      : [0, 0, 0, 0],
  };
}
const rasterModule = {
...
function getUniforms$2(props = {}) {
  return {
    ...
    [tokens$2.paletteColor]: props.paletteColor
      ? deckColorToGl(props.paletteColor)
      : [0, 0, 0, 0],
  };
}
const paletteModule = {
...
class RasterBitmapLayer extends BitmapLayer {
  getShaders() {
  ...
  draw(opts) {
    ...
    const {
      ...
      gridSize,
      gridColor,
    } = ensureDefaultProps(this.props, defaultProps$b);
  ...
  _updatePalette() {
    ...
    const { paletteBounds, paletteTexture } = createPaletteTexture(
      device,
      paletteScale
    );
  ...
  _getRasterMagnitudeValue(color, paletteBounds) {
    return (
      paletteBounds[0] +
      (color[0] / 255) * (paletteBounds[1] - paletteBounds[0])
    );
  }
  ...
  getPickingInfo(params) {
    ...
    const value = this._getRasterMagnitudeValue(
      info.color,
      paletteBounds ?? [0, 0]
    );
    if (imageType === ImageType.VECTOR) {
  ...
  updateState(params) {
    ...
    if (image !== params.oldProps.image || image2 !== params.oldProps.image2) {
      ...
      const imageTexture = image
        ? createTextureCached(device, image, isRepeatBounds(bounds))
        : null;
      const imageTexture2 = image2
        ? createTextureCached(device, image2, isRepeatBounds(bounds))
        : null;
...
// eslint-disable
const sourceCode$3 =
  "uniform contourUniforms{float interval;float majorInterval;float width;}contour;";
const tokens$1 = {};
...
class ContourBitmapLayer extends BitmapLayer {
  getShaders() {
    const parentShaders = super.getShaders();
    return {
      ...parentShaders,
  ...
  draw(opts) {
    ...
    const {
      ...
      majorInterval,
      width,
    } = ensureDefaultProps(this.props, defaultProps$9);
    const { paletteTexture, paletteBounds } = this.state;
    if (!imageTexture) {
      return;
    }
...
const workerCode = transform(
  getWorkerString(function () {
    ...
    function getHighLowPointDataMain(imageProperties, bounds, radius) {
      ...
      const highLowPointData = getHighLowPointData(
        magnitudeData,
        bounds,
        radius
      );
    ...
    expose({
      ...
      getHighLowPointData(
        ...
        const highLowPointData = getHighLowPointDataMain(
          imageProperties,
          bounds,
          radius
        );
...
function getHighLowPointsFromData(highLowPointData) {
  ...
  for (let j = 0; j < highCount; j++) {
    ...
    highLowPoints.push(
      createHighLowPoint(position, { type: HighLowType.HIGH, value })
    );
  }
  ...
  for (let j = 0; j < lowCount; j++) {
    ...
    const value = highLowPointData[i++];
    highLowPoints.push(
      createHighLowPoint(position, { type: HighLowType.LOW, value })
    );
  }
...
async function getHighLowPoints(imageProperties, bounds, radius) {
  ...
  const highLowPointData = await highLowPointWorkerProxy.getHighLowPointData(
    transfer(dataCopy, [dataCopy.buffer]),
    width,
    height,
    data2Copy ? transfer(data2Copy, [data2Copy.buffer]) : null,
    width2,
    height2,
    imageSmoothing,
    imageInterpolation,
...
function getHighLowPointCollisionPriority(highLowPoint, minValue, maxValue) {
  if (highLowPoint.properties.type === HighLowType.HIGH) {
    return Math.round(
      ((highLowPoint.properties.value - maxValue) / maxValue) * 100
    );
  } else {
    return Math.round(
      ((minValue - highLowPoint.properties.value) / minValue) * 100
    );
  }
}
...
class HighLowCompositeLayer extends CompositeLayer {
  renderLayers() {
    ...
    const {
      ...
      textOutlineWidth,
      textOutlineColor,
    } = ensureDefaultProps(props, defaultProps$7);
    ...
    return [
      new TextLayer(
        this.getSubLayerProps({
          ...
          getPixelOffset: [
            0,
            -getViewportPixelOffset(viewport, (textSize * 1.2) / 2),
          ],
          ...
          getColor: (d) =>
            paletteScale
              ? paletteColorToGl(paletteScale(d.properties.value).rgba())
              : textColor,
          ...
          getCollisionPriority: (d) =>
            getHighLowPointCollisionPriority(d, minValue, maxValue),
          parameters: {
  ...
  shouldUpdateState(params) {
    return (
      super.shouldUpdateState(params) || params.changeFlags.viewportChanged
    );
  }
  updateState(params) {
    const {
  ...
  async _updateFeatures() {
    const {
      ...
      bounds,
      radius,
    } = ensureDefaultProps(this.props, defaultProps$7);
    if (!image) {
      return;
    }
    ...
    const points = (await getHighLowPoints(imageProperties, bounds, radius))
      .features;
  ...
  _redrawVisibleFeatures() {
    this.setState({
      visiblePoints: Array.isArray(this.state.visiblePoints)
        ? Array.from(this.state.visiblePoints)
...
function getFrontLine(d, path) {
  ...
  const distances = positions
    .slice(0, -1)
    .map((_, i) => distance(positions[i], positions[i + 1]));
  const cummulativeDistances = distances.reduce(
  ...
  let icons = [];
  for (
    let depth = 1, deltaDistance = totalDistance / ICON_FACTOR;
    deltaDistance > ICON_MIN_DISTANCE;
    depth++, deltaDistance /= ICON_FACTOR
  ) {
    ...
    for (let i = 1; i < iconCountAtDepth; i++) {
      ...
      const positionStartIndex = findLastIndex(
        cummulativeDistances,
        (x) => x <= distance
      );
      if (
        positionStartIndex === -1 ||
        positionStartIndex === positions.length - 1
      ) {
  ...
  const alternatingIcons = icons.map((icon, i) => ({
    d,
    ...icon,
    alternate: i % 2 === 0,
  }));
...
class FrontCompositeLayer extends CompositeLayer {
  renderLayers() {
    ...
    const {
      ...
      iconSize,
      _debug: debug,
    } = ensureDefaultProps(props, defaultProps$5);
    if (!getType || !getPath) {
      return [];
    }
    ...
    return [
      new PathLayer(
        this.getSubLayerProps({
          ...
          getPath: (d) => [
            d.startPosition,
            ...d.icons.map((point) => point.position),
            d.endPosition,
          ],
      ...
      new PathLayer(
        this.getSubLayerProps({
          ...
          data: visibleFrontLines.filter(
            (d) => getType(d.d) === FrontType.STATIONARY
          ),
          getPath: (d) => [
            d.startPosition,
            ...d.icons.map((point) => point.position),
      ...
      new IconLayer(
        this.getSubLayerProps({
          ...
          getIcon: (d) =>
            getType(d.d) === FrontType.OCCLUDED ||
            getType(d.d) === FrontType.STATIONARY
              ? d.alternate
                ? FrontType.COLD
                : FrontType.WARM
              : getType(d.d),
  ...
  updateState(params) {
    ...
    if (data !== params.oldProps.data || getPath !== params.oldProps.getPath) {
      this._updateFeatures();
    }
    if (
      minZoom !== params.oldProps.minZoom ||
  ...
  async _updateIconStyle() {
    ...
    const iconAtlasTexture = createTextureCached(
      device,
      await loadTextureData(iconStyle.iconAtlas)
    );
  ...
  _updateFeatures() {
    ...
    const debugFrontPoints = data.flatMap((d) =>
      getPath(d).map((position, index) => ({ d, position, index }))
    );
...
function generateGlobeGrid(center, radius, zoom) {
  ...
  const globalPositions =
    GLOBAL_POSITIONS_AT_ZOOM_CACHE.get(zoom) ??
    (() => {
  ...
  const globalIndex =
    GLOBAL_INDEX_AT_ZOOM_CACHE.get(zoom) ??
    (() => {
      const globalIndex = new KDBush(
        globalPositions.length,
        undefined,
        Float32Array
      );
...
function generateGrid(bounds, zoom) {
  ...
  const gridBounds = [
    ...mercator.px([bounds[0], bounds[1]], zoom),
    ...mercator.px([bounds[2], bounds[3]], zoom),
  ];
...
class GridCompositeLayer extends CompositeLayer {
  renderLayers() {
    ...
    const {
      ...
      iconSize,
      iconColor,
    } = ensureDefaultProps(props, defaultProps$3);
    const { paletteScale } = this.state;
    if (GRID_ICON_STYLES.has(style)) {
      ...
      const iconBoundsRatio = (value) =>
        (value - iconBounds[0]) / iconBoundsDelta;
      const iconSizeDelta = Array.isArray(iconSize)
        ...
        new IconLayer(
          this.getSubLayerProps({
            ...
            getIcon: (d) =>
              `${Math.min(
                Math.max(
                  Math.floor(iconBoundsRatio(d.properties.value) * iconCount),
                  0
                ),
                iconCount - 1
              )}`,
            getSize: (d) =>
              Array.isArray(iconSize)
                ? iconSize[0] +
                  iconBoundsRatio(d.properties.value) * iconSizeDelta
                : iconSize,
            getColor: (d) =>
              paletteScale
                ? paletteColorToGl(paletteScale(d.properties.value).rgba())
            ...
            getAngle: (d) =>
              getViewportAngle(
                viewport,
                d.properties.direction ? 360 - d.properties.direction : 0
              ),
            iconAtlas: iconAtlasTexture,
            iconMapping: iconStyle.iconMapping,
            billboard: false,
  ...
  updateState(params) {
    ...
    if (!this.state.iconStyle || style !== params.oldProps.style) {
      this._updateIconStyle();
    }
    if (
  ...
  _updateFeatures() {
    const {
      ...
      imageMaxValue,
      bounds,
    } = ensureDefaultProps(this.props, defaultProps$3);
    ...
    const points = getRasterPoints(
      imageProperties,
      bounds,
      positions
    ).features.filter((d) => !isNaN(d.properties.value));
...
// eslint-disable
const sourceCode$1 =
  "uniform particleUniforms{float viewportGlobe;vec2 viewportGlobeCenter;float viewportGlobeRadius;vec4 viewportBounds;float viewportZoomChangeFactor;float numParticles;float maxAge;float speedFactor;float time;float seed;}particle;";
const tokens = {};
...
class ParticleLineLayer extends LineLayer {
  getShaders() {
    const parentShaders = super.getShaders();
    return {
      ...parentShaders,
  ...
  updateState(params) {
    const { imageType, numParticles, maxAge, width, palette, visible } =
      params.props;
    super.updateState(params);
    if (!visible) {
  ...
  _setupTransformFeedback() {
    ...
    const { numParticles, maxAge } = ensureDefaultProps(
      this.props,
      defaultProps$1
    );
    ...
    const sourceColors = device.createBuffer(
      new Float32Array(numInstances * 4)
    );
    const targetColors = device.createBuffer(
      new Float32Array(numInstances * 4)
    );
  ...
  _runTransformFeedback() {
    ...
    const {
      ...
      maxAge,
      speedFactor,
    } = ensureDefaultProps(this.props, defaultProps$1);
    const {
      paletteTexture,
      paletteBounds,
      numAgedInstances,
    ...
    if (
      ...
      time < previousTime + 1000 / FPS$1
    ) {
      return;
    }
    // viewport
    const viewportGlobe = isViewportGlobe(viewport);
    const viewportGlobeCenter = isViewportGlobe(viewport)
      ? getViewportGlobeCenter(viewport)
      : undefined;
    const viewportGlobeRadius = isViewportGlobe(viewport)
      ? getViewportGlobeRadius(viewport)
      : undefined;
    const viewportBounds = isViewportMercator(viewport)
      ? getViewportBounds(viewport)
      : undefined;
    const viewportZoomChangeFactor =
      2 **
      ((typeof previousViewportZoom === "number"
        ? previousViewportZoom - getViewportZoom(viewport)
        : 0) *
        4);
    // speed factor for current zoom level
    const currentSpeedFactor =
      speedFactor / 2 ** (getViewportZoom(viewport) + 7);
...
class LegendControl extends Control {
  ...
  setConfig(config) {
    ...
    for (let i = 0; i < ticksCount; i++) {
      ...
      tick.style.transform = `translate(${
        ((value - paletteBounds[0]) / (paletteBounds[1] - paletteBounds[0])) *
        100
      }%, 0)`;
...
class TooltipControl extends Control {
  ...
  update(rasterPointProperties) {
    ...
    this._container.classList.toggle(
      FOLLOW_CURSOR_CLASS,
      this._config.followCursor ?? false
    );
    this._container.setAttribute(
      FOLLOW_CURSOR_PLACEMENT_ATTRIBUTE,
      this._config.followCursorPlacement ?? Placement.BOTTOM
    );
    this._container.classList.toggle(
      HAS_VALUE_CLASS,
    ...
    if (typeof value !== "undefined") {
      this._value.innerHTML = formatValueWithUnit(
        value,
        this._config.unitFormat
      );
    } else {
      this._value.innerHTML = "";
    }
    if (typeof direction !== "undefined") {
      this._directionIcon.style.transform = `rotate(${
        (direction + 180) % 360
      }deg)`;
      this._directionText.innerHTML = formatDirection(
        direction,
        this._config.directionType ?? DirectionType.INWARD,
        this._config.directionFormat ?? DirectionFormat.VALUE
      );
    } else {
  ...
  updatePickingInfo(pickingInfo) {
    ...
    if (this._config.followCursor) {
      ...
      if (
        ...
        const divPaddingLeft = parseFloat(
          window.getComputedStyle(div).paddingLeft
        );
        const directionMarginLeft = parseFloat(
          window.getComputedStyle(this._direction).marginLeft
        );
        const divX = -(
          divPaddingLeft +
          (hasDirection
            ? valueBounds.width + directionMarginLeft
            : valueBounds.width / 2)
        );
        div.style.left = `${divX}px`;
      }
      if (
        followCursorPlacement === Placement.RIGHT ||
        followCursorPlacement === Placement.LEFT
      ) {
      ...
      // hide on panning
      document.addEventListener("mousedown", () => this.update(undefined), {
        once: true,
      });
    } else {
...
class TimelineControl extends Control {
  ...
  get _startEndDatetimes() {
    ...
    const startDatetime =
      this._config.datetimes[Math.floor(this._progressInput.valueAsNumber)];
    const endDatetime =
      this._config.datetimes[Math.ceil(this._progressInput.valueAsNumber)];
  ...
  _updateProgress() {
    ...
    const datetimeFormatFunction =
      this._config.datetimeFormatFunction ?? formatDatetime;
  ...
  setConfig(config) {
    ...
    if (
      ...
      this._config.datetimes.every(
        (datetime, i) => datetime === config.datetimes[i]
      ) &&
    ...
    const datetimeEndIndex =
      datetimeStartIndex < datetimes.length - 1 ? datetimeStartIndex + 1 : null;
    const datetimeStart = datetimes[datetimeStartIndex];
    const datetimeEnd =
      typeof datetimeEndIndex === "number"
        ? datetimes[datetimeStartIndex + 1]
        : null;
    const datetimeWeight = getDatetimeWeight(
      datetimeStart,
      datetimeEnd,
      datetime
    );
    ...
    const progressInputValue =
      datetimeStartIndex +
      Math.floor(datetimeWeight / progressInputStep) * progressInputStep;
    this._container.innerHTML = "";
    this._container.style.width = `${width}px`;
    const div = document.createElement("div");
    this._container.appendChild(div);
    ...
    endDatetime.innerHTML = datetimeFormatFunction(
...
export {
  ...
  DATETIME,
  DEFAULT_ICON_COLOR,
  DEFAULT_ICON_SIZE$1 as DEFAULT_ICON_SIZE,
  DEFAULT_LINE_COLOR,
  DEFAULT_LINE_WIDTH,
  DEFAULT_TEXT_COLOR,
  ...
  formatValue,
  formatValueWithUnit,
  getClosestEndDatetime,
  getClosestStartDatetime,
  getDatetimeWeight,
  getRasterMagnitudeData,
  getRasterPoints,
  interpolateDatetime,
  loadJson,
  loadTextureData,
