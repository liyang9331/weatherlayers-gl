...
function getStacCollectionAttribution(stacCollection, attributionLinkClass) {
  const producer = stacCollection.providers.find((x) =>
    x.roles.includes(StacProviderRole.PRODUCER)
  );
  const processor = stacCollection.providers.find((x) =>
    x.roles.includes(StacProviderRole.PROCESSOR)
  );
  const attribution = [
    ...(producer
      ? [
          `<a href="${producer.url}"${
            attributionLinkClass ? ` class="${attributionLinkClass}"` : ""
          }>${producer.name}</a>`,
        ]
      : []),
    ...(processor
      ? [
          `<a href="${processor.url}"${
            attributionLinkClass ? ` class="${attributionLinkClass}"` : ""
          }>${processor.name}</a>`,
        ]
      : []),
...
function getStacCollectionUnitFormat(stacCollection, unitSystem) {
  const units = stacCollection["units"];
  const unitDefinition =
    units.find((unitFormat) => unitFormat.system === unitSystem) ??
    units.find((unitFormat) => unitFormat.system === DEFAULT_UNIT_SYSTEM) ??
    units[0];
...
class Client {
  ...
  _cacheDatasetDataStacItem(dataset, stacItem) {
    ...
    this._datasetDataStacItemCache
      .get(dataset)
      .set(stacItem.properties.datetime, stacItem);
  }
  async _loadStacCatalog(config = {}) {
    const url = config.url ?? this._config.url ?? DEFAULT_URL;
    const authenticatedUrl = this._getAuthenticatedUrl(
      `${url}/catalog`,
      config
    );
    const stacCatalog = await loadJson(authenticatedUrl, {
      cache: this._cache,
    });
    return stacCatalog;
  }
  async _loadDatasetStacCollections(config = {}) {
    ...
    const stacCollections = (
      await loadJson(authenticatedUrl, { cache: this._cache })
    ).collections;
    // cache
  ...
  async _loadDatasetStacCollectionPalette(dataset, config = {}) {
    const stacCollection = await this._loadDatasetStacCollection(
      dataset,
      config
    );
    const asset = Object.values(stacCollection.assets ?? {}).find(
      (x) =>
        x.roles.includes(StacAssetRole.PALETTE) && x.type === "application/json"
    );
    if (!asset) {
  ...
  async _searchDatasetDataStacItems(
    dataset,
    datetimeRange,
    datetimeStep,
    config = {}
    ...
    url.searchParams.set(
      "datetime",
      serializeDatetimeISOStringRange(datetimeRange)
    );
    ...
    const stacItems = (await loadJson(authenticatedUrl, { cache: this._cache }))
      .features;
    // cache
  ...
  async _loadDatasetDataStacItem(dataset, datetime, config = {}) {
    ...
    if (!stacItem) {
      const stacItems = await this._searchDatasetDataStacItems(
        dataset,
        [datetime, datetime],
        datetimeStep,
        config
  ...
  async _loadStacItemData(stacItem, config = {}) {
    const dataFormat =
      config.dataFormat ?? this._config.dataFormat ?? DEFAULT_DATA_FORMAT;
    ...
    const image = await loadTextureData(authenticatedUrl, {
      cache: this._cache,
  ...
  async _loadDatasetDataStacItemDataNow(dataset, config = {}) {
    ...
    const link = stacCollection.links.find(
      (x) => x.rel === StacLinkRel.ITEM && x.datetime === NOW_DATETIME
    );
    if (!link) {
  ...
  async loadCatalog(config = {}) {
    ...
    const datasetIds = stacCollections.map(
  ...
  async loadDataset(dataset, config = {}) {
    ...
    const unitSystem =
      config.unitSystem ?? this._config.unitSystem ?? DEFAULT_UNIT_SYSTEM;
    const attributionLinkClass =
      config.attributionLinkClass ??
      this._config.attributionLinkClass ??
      DEFAULT_ATTRIBUTION_LINK_CLASS;
    return {
      title: stacCollection.title,
      unitFormat: getStacCollectionUnitFormat(stacCollection, unitSystem),
      attribution: getStacCollectionAttribution(
        stacCollection,
        attributionLinkClass
      ),
      bounds: stacCollection.extent.spatial.bbox[0],
      datetimeRange: stacCollection.extent.temporal.interval[0],
      datetimes: stacCollection.links
        .filter((x) => x.rel === StacLinkRel.ITEM)
        .map((x) => x.datetime)
        .filter((x) => !!x),
  ...
  async loadDatasetData(dataset, datetime, config = {}) {
    const datetimeStep = config.datetimeStep ?? this._config.datetimeStep ?? 1;
    const datetimeInterpolate =
      config.datetimeInterpolate ?? this._config.datetimeInterpolate ?? false;
    const stacCollection = await this._loadDatasetStacCollection(
      dataset,
      config
    );
      ...
      return {
        ...
        imageUnscale:
          ...
          data.image.data instanceof Uint8ClampedArray
            ? stacCollection["weatherLayers:imageUnscale"]
            : null,
        bounds: stacCollection.extent.spatial.bbox[0],
      };
    }
    let stacItems = this._datasetDataStacItemCache.has(dataset)
      ? Array.from(this._datasetDataStacItemCache.get(dataset).values())
      : [];
    let datetimes = stacItems.map((x) => x.properties.datetime).sort();
    if (
      !datetimes.length ||
      datetimes[0] > datetime ||
      datetimes[datetimes.length - 1] < datetime
    ) {
      stacItems = await this._searchDatasetDataStacItems(
        dataset,
        [datetime, datetime],
        datetimeStep,
        config
    ...
    if (
      datetimeInterpolate &&
      closestStartDatetime &&
      closestEndDatetime &&
      closestStartDatetime !== closestEndDatetime
    ) {
    ...
    const [data, data2] = await Promise.all([
      ...
      datetimeInterpolate && endDatetime
        ? this._loadDatasetDataStacItemData(dataset, endDatetime, config)
        : null,
    ]);
    ...
    return {
      ...
      imageWeight: data2
        ? getDatetimeWeight(data.datetime, data2.datetime, datetime)
        : 0,
      imageType: stacCollection["imageType"],
      imageUnscale:
        data.image.data instanceof Uint8Array ||
        data.image.data instanceof Uint8ClampedArray
          ? stacCollection["imageUnscale"]
          : null,
      bounds: stacCollection.extent.spatial.bbox[0],
    };
  }
