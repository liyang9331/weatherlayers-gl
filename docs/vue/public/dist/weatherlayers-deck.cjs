...
var browser_js = require('rollup-plugin-worker-factory/src/browser.js');
var sphericalmercator = require('@mapbox/sphericalmercator');
var icomesh = require('icomesh');
var KDBush = require('kdbush');
var geokdbush = require('geokdbush');
var engine = require('@luma.gl/engine');
...
var geokdbush__namespace = /*#__PURE__*/_interopNamespaceDefault(geokdbush);

// TODO: fix Rollup build config to use TS instead of JS
const VERSION = "2025.8.0";
const DATETIME = "2025-09-15T08:34:52.653Z";
...
const workerCode = browser_js.transform(getWorkerString(function () {
...
var createHighLowPointWorker = browser_js.factory(workerCode);
...
const highLowPointWorkerProxy = comlink.wrap(createHighLowPointWorker());
...
async function getHighLowPoints(imageProperties, bounds, radius) {
    ...
    const highLowPointData = await highLowPointWorkerProxy.getHighLowPointData(comlink.transfer(dataCopy, [dataCopy.buffer]), width, height, data2Copy ? comlink.transfer(data2Copy, [data2Copy.buffer]) : null, width2, height2, imageSmoothing, imageInterpolation, imageWeight, imageType, imageUnscale, imageMinValue, imageMaxValue, bounds, radius);
...
function getFrontLine(d, path) {
    ...
    for (let depth = 1, deltaDistance = totalDistance / ICON_FACTOR; deltaDistance > ICON_MIN_DISTANCE; depth++, deltaDistance /= ICON_FACTOR) {
        ...
        for (let i = 1; i < iconCountAtDepth; i++) {
            ...
            const bearing = geodesyFn.initialBearing(positionStart, positionEnd);
...
function generateGlobeGrid(center, radius, zoom) {
    ...
    const ids = geokdbush__namespace.around(globalIndex, center[0], center[1], undefined, radius / 1000);
    const positions = ids.map(i => globalPositions[i]);
    return positions;
}
function generateGrid(bounds, zoom) {
    const mercator = new sphericalmercator.SphericalMercator({ size: 1, antimeridian: true });
...
Object.defineProperty(exports, "colorRampCanvas", {
  enumerable: true,
  get: function () { return cpt2js.colorRampCanvas; }
});
Object.defineProperty(exports, "parsePalette", {
  enumerable: true,
  get: function () { return cpt2js.parsePalette; }
});
